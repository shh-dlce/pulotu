...
from website.apps.core.models import Culture, Section, Source
...
class TextResponseForm(ResponseForm):
	response = forms.CharField(required=False, widget=forms.widgets.Textarea(attrs={'class': 'span6', 'rows': '3'}))
	missing = forms.BooleanField(required=False)
	class Meta:
		model = TextResponse
		hidden = ('id', 'question', 'culture')
...
class OptionResponseForm(ResponseForm):
	...
	response = forms.ChoiceField(required=False, choices=(), widget=forms.RadioSelect(attrs={'class':'radios'}),)
	...
	def clean(self):
		...
		if not response and not missing: #if no response is given, raise an error
			raise forms.ValidationError("Response is a required field.")
		if not missing and response and not source1: #if response is given and no source is given
			raise forms.ValidationError("Source 1 is a required field.")
		return cleaned_data
...
def construct_section_forms(post_data=None, culture_obj=None, section_obj=None):
	...
	responses = {}
	source1 = {}
	for r in Response.objects.all().filter(culture=culture_obj, question__section=section_obj):
