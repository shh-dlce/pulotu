...
from website.apps.survey.models import Response,OptionResponse, FloatResponse, IntegerResponse, TextResponse
...
class QuestionAdmin(TrackedModelAdmin, VersionAdmin, PolymorphicParentModelAdmin):
	...
	ordering = ('number',)
	#search_fields = ('question', 'information')
	#list_display = ('number', 'section', 'question',)
	child_models = [
		...
		if 'cancel' in request.POST:
			...
			if form.is_valid():
				...
				for link in queryset:
					link.number += offset
					link.save()
				self.message_user(request, self.changeSuccess.render(Context({'count':queryset.count()})))
				return HttpResponseRedirect(request.get_full_path())
		if not form:
			form = self.NumberForm(initial={'_selected_action': request.POST.getlist(admin.ACTION_CHECKBOX_NAME)})
		return render_to_response('admin/categorize.html', RequestContext(request, {'links': queryset, 'form': form, 'path':request.get_full_path()}))
		...
		if 'cancel' in request.POST:
			...
			if form.is_valid():
				...
				for link in queryset:
					...
					link.save()
				self.message_user(request, self.categorySuccess.render(Context({'count':queryset.count(), 'category':section})))
				return HttpResponseRedirect(request.get_full_path())
		if not form:
			form = self.CategoryForm(initial={'_selected_action': request.POST.getlist(admin.ACTION_CHECKBOX_NAME)})
		return render_to_response('admin/change.html', RequestContext(request, {'links': queryset, 'form': form, 'path':request.get_full_path()}))
...
class ResponseAdmin(TrackedModelAdmin, VersionAdmin):
	#form = make_ajax_form(Response, {'source':'source1'})
	date_hierarchy = 'added'
...
#		if change:
#			for t in form.cleaned_data['source']:
#				self.message_user(request, 'source is {{t}}')
