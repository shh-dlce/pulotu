...
class Question(PolymorphicModel, TrackedModel):
    ...
    publicNumber = models.IntegerField(
        blank=True,
        null=True,
        verbose_name="Public Number",
        help_text="For public order of questions")

    number = models.IntegerField(
        db_index=True,
        help_text="The sequence number for this question")  # should this be unique?
    question = models.CharField(
        max_length=255,
        db_index=True,
        unique=True,
        help_text="The Question. Anything you do not want visible to the general public"
                  " should go in the Information section.")

    simplified_question = models.CharField(max_length=255, blank=True, null=True)
    information = models.TextField(
        blank=True, help_text="Information to display to the coder")
...
class Response(ShowFieldType, PolymorphicModel):
    ...
    def __unicode__(self):
        if not self.pk:
            ...
            return u"Response: {0}-{1}-{2}: {3}".format(
                self.question.id, self.culture_id, self.source1, self.response)
        else:
...
class OptionResponse(Response):
    ...
    response_text = models.TextField(
        blank=True,
        null=True,
        help_text="The text response from the user. "
                  "We save this only for debugging purposes.")
