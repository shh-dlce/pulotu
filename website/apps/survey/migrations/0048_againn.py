...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		...
		for response in orm.Response.objects.all():
				response.source1 = response.source.all()[0]
				try:
					response.source2 = response.source.all()[1]
				except:
					continue
				try:
					response.source3 = response.source.all()[2]
				except:
					continue
				try:
					response.source4 = response.source.all()[3]
				except:
					continue
				try:
					response.source5 = response.source.all()[4]
				except:
					continue
				response.save()
