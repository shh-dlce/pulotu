...
QUESTIONS[5] = {
    'section': 'physical-environment',
    'question': 'Frequency of tropical cyclones',
    'information': '',
    'options': dedent("""
    (?) Missing data
	(0) Very low (Cyclones never occur, or are very rare)
	(1) Low (Cyclones are rare, occurring perhaps once in a generation)
	(2) Moderate (Cyclones occur more than once in a generation, but most years are cyclone-free)
	(3) High (Cyclones occur most years, or every year) 
    """),
}
QUESTIONS[7] = {
    'section': 'physical-environment',
    'question': 'Earthquakes (within the past 12 months and a 250km radius, Richter Scale)',
    'information': '',
    'options': dedent("""
    (?) Missing data
    (0) Absent, no earthquakes reported or less than 4.0
    (1) Present, at least one earthquake between 4.0 and 5
    (2) Present, at least one earthquake between  5.1 and 6.0
    (3) Present, at least one earthquake over 6.0
    """),
}
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        ctype = orm['contenttypes.contenttype'].objects.all()
        ctype = ctype.filter(app_label='survey', model='optionquestion').get()
        for qnum, qdata in QUESTIONS.iteritems():
            q = orm.OptionQuestion.objects.create(
                section=Section.objects.get(slug=qdata['section']),
                number=qnum,
                question=qdata['question'],
                information=qdata['information'],
