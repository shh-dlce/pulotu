...
from website.apps.core.models import Section, Culture, Source, Category
from website.apps.survey.models import Question, Response
from website.apps.survey.forms import construct_section_forms, OptionResponseForm, get_form_type, ExportForm
from website.apps.survey.tables import SurveyIndexTable, SurveyCultureIndexTable
from django.forms.formsets import formset_factory, BaseFormSet
from django.core.context_processors import csrf
from django.core import serializers
from django.http import HttpResponse, HttpResponseRedirect
import csv, codecs, cStringIO, StringIO
import json
...
def indexx(request, slug):		
		...
		for s in subsections:
			for q in questions.filter(subsection=s):
				if q.section not in subDict[str(q.subsection)]:
					...
					q.section.questions = question.get(q.section.id, 0)-q.section.answered
	...
	return render_to_response('survey/survey_culture_index1.html', {
		'culture': dict(fullDict), 'full':sections, 'object':culture,
	}, context_instance=RequestContext(request))

	#keeping this code for emergencies, if the above view fails at some point
	#the table it produces is messy but guaranteed to function
class SurveyCultureIndex(DetailView):
	...
	def get_context_data(self, **kwargs):
		...
		responses = {}
		missing = {}
		resp = Response.objects.filter(culture=kwargs['object']).select_related('question')
		for r in resp:
			sect_id = r.question.section_id
			responses[sect_id] = responses.get(sect_id, 0) + 1
			if r.missing:
	...
	if request.method == 'POST':
		...
		for form in forms:
			if form.is_valid():
				...
				obj.save()
				status.append("Saved Response to question {0}.{1}".format(section_obj.id, obj.question.number))
			else:
				all_valid = False		
		# if all forms are valid. Return to survey culture index
		if all_valid:
			return HttpResponseRedirect(reverse('survey-culture-index', kwargs={"slug": culture} ))
...
def download_dataset(request):
    ...
    if request.user.is_authenticated():
        ...
        if not questionlist:
            ...
            questions = Question.objects.all().filter(id__in=questionlist).order_by('number')
            ...
            for q in questions:
                ...
                if q.simplified_question:
                    headings.append('v'+str(q.number)+'.'+q.simplified_question.replace(' ','_'))
                else:
        ...
        questions = Question.objects.all().filter(displayPublic=False).order_by('number') #get all public questions
        for q in questions:
            if q.simplified_question:
                headings.append('v'+str(q.number)+'.'+q.simplified_question.replace(' ', '_'))
            else:
...
def chooseexport(request):
    ...
    return render_to_response('survey/excel_format.html', {'questions':OrderedDict(fullDict), 'exportinfo':''}, context_instance=RequestContext(request))			
...
def download_references(request):
	if cache.get('refs') is not None:
		...
		writer = UnicodeWriter(response, delimiter='#', quoting=csv.QUOTE_NONE, escapechar=' ')
		allresponses = Response.objects.all().select_related('culture')
		for culture in Culture.objects.all():
			resps = allresponses.filter(culture=culture).select_related('source1', 'source2', 'source3', 'source4', 'source5')
			refs = set()
			for r in resps:
