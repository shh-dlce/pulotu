...
class Source(TrackedModel):
	...
	author = models.CharField(max_length=255, db_index=True, 
		help_text="Short Author list.  If the source has 2 authors, state this as Smith & Johnson.  If the source has more than 2 authors, state Smith et al.")
	slug = models.SlugField(max_length=1000, null=False, unique=True,
		help_text="`Slug` for author i.e. author-year (for use in URLS)")
	reference = models.TextField(blank = True, null=True,
		help_text="APA reference for Source.")
...
class Language(TrackedModel):
	...
	#@models.permalink
	#def get_absolute_url(self):
	#	return ('language-detail', [self.isocode])
...
class Category(TrackedModel):
	...
	timeFocus = models.BooleanField("No time focus associated with this category", default=False)
	def __unicode__(self):
...
class Section(TrackedModel):
	category = models.ForeignKey(Category, blank=True, null=True,
		help_text="Category that this Section belongs to")
	
	section = models.CharField(max_length=128)
	
	slug = models.SlugField(max_length=128, unique=True,
		help_text="`Slug` for this section (for use in URLS)"
	)
	notes = models.TextField(blank=True, null=True, 
		help_text="Public Notes on this section"
	)
...
class Culture(TrackedModel):
	...
	culture = models.CharField(max_length=128, db_index=True, unique=True,
		help_text="Name of culture",
	)
	slug = models.SlugField(max_length=128,
		help_text="`Slug` for this section (for use in URLS)"
	)
	notes = models.TextField(blank=True, null=True, 
		help_text="Brief description of culture (2-3 sentences)"
	)
	
	#need to change this name to ethnoyms!
	ethonyms = models.TextField("Ethonyms", blank=True, null=True, 
		help_text="Ethonyms for this culture. Please separate ethonyms with a semicolon and space, e.g. 'Ethonym 1; Ethonym 2, East' (without the quotation marks). Do not enter the culture's name as an ethonym. Do not use a semicolon within an ethonym.")
	
	languages = models.ManyToManyField(Language,
		help_text="The languages affiliated with this culture."
	)
	
	coder = models.CharField(max_length=256, blank=True, null=True, 
		help_text="Coder's full name")
	def __unicode__(self):
		return self.culture
...
class Publication(TrackedModel):
	link = models.CharField(max_length=128, unique=True, blank=True, null=True,
		help_text="Link to this publication. Must be a complete link (not just a doi). Leave blank if no link to article.")
	reference = models.TextField(
		help_text="Full APA reference for this publication.")
	def __unicode__(self):
		return self.reference
	
watson.register(Glossary, TermSearchAdapter)
watson.register(Culture, SearchAdapter, fields=("culture", "ethonyms", "languages__isocode", "languages", "languages__abvdcode", "coder",), exclude=("notes",))
